        ...
        <div class="Main">
            ...
            <hr class="Horizontal">
                @foreach($response->getUser()->project_app as $application)
                    <a class="fw-bold d-block" href="{{ route('front.project.detail', ['id' => $application->id] )}}">{{$application->name ?? ''}}</a>
                @endforeach
            <p class="fw-bold fs-20 mt-20">稼働案件</p>
            <hr class="Horizontal">
                @foreach($response->getUser()->project_assign as $assignment)
                    <a class="fw-bold d-block" href="{{ route('front.project.detail', ['id' => $assignment->id] )}}">{{$assignment->name ?? ''}}</a>
                @endforeach
        </div>
    </div>
