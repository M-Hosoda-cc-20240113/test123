...
<form class="formrun" action="https://form.run/api/v1/r/frzv3ab5jrd83ayr2qsfbr2h" method="post">
  ...
  <input class="c-input u-mt-5" name="お名前"
         value="{{ $response->getUser()->sei ?? '' }}&#160;{{ $response->getUser()->mei ?? '' }}" type="text"
         placeholder="お名前入力欄">
  {{-- お名前 --}}

  <p class="c-text u-mt-20">メールアドレス<span class="c-label--sm c-label--warning u-ml-10">必須</span></p>
  <input placeholder="tanaka@example.com" value="{{ $response->getUser()->email ?? '' }}" class="c-input u-mt-5"
         name="メールアドレス" type="text" data-formrun-type="email" data-formrun-required>
  ...
  <div class="_formrun_gotcha">
    <style media="screen">._formrun_gotcha {
  ...
  <button class="c-button u-mt-20" type="submit" data-formrun-error-text="未入力の項目があります"
          data-formrun-submitting-text="送信中...">送信
  </button>
</form>
