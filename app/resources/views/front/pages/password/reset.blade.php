...
@section('title', 'パスワード再設定 |【Aegis(イージス)】フリーランス・ITエンジニアのための案件サイト')
...
@section('body')
  ...
  <div class="l-container">
    <form action="{{ route('password.request') }}" method="post">
      ...
      <p class="c-text">半角英数字8文字以上で大文字を1つ以上含むパスワードを入力してください。</p>
      @foreach($errors->all() as $error)
        <p class="c-text--warning">{{ $error }}</p>
      @endforeach
      <input class="c-input--light u-mt-20" type="password" name="password" placeholder="パスワード入力欄">
      <input class="c-input--light u-mt-10" type="password" name="password_confirmation"
             placeholder="パスワード入力欄(確認)">
